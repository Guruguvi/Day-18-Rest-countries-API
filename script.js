...
async function getCountries(){
   const all = await fetch("https://restcountries.com/v2/all");
...
//To create a CARD
   ...
   card1.innerHTML = ` 
       <div class="card-header">
           <h4 class="card-text">${country.name}</h4>
       </div>
       ...
       <div class="card-body">
         <div class="card-text">
           ...
           <h6>Country Code: ${country.cioc}</h6>
         </div>
       </div>`;
...
//To get all country WEATHER REPORT details from OPENWEATHERMAP API by using async/await and fetch()
 async function weatherReport(){
   const weather = await fetch (`https://api.openweathermap.org/data/2.5/weather?q=${country.name}&appid=ca112fd8ff007a6940a08407c211d787`);
